...
5. Each measurement runs 1000 trials over those 50 states — **50 000 contact checks** — and
   ...
   form, so they measure 49 000 checks.
...
| `--clone` | off | clone the manager per state pair in continuous mode |
...
- **`--clone` only affects continuous mode.** It clones the contact manager for every state pair
  instead of reusing one, which is how TrajOpt drives the managers; it measures setup cost that a
  reused manager amortises away.

## CSV output

```
scenario,manager,mode,checks_per_second,total_num_checks,num_contacts
```
...
`checks_per_second_<scenario>.png`, plus a combined `checks_per_second_all_scenarios.png`. Each
figure has one subplot per contact test type, one bar per manager, using the mean of
`checks_per_second` where a scenario/type/manager appears more than once. Colours are fixed per
manager, so they hold across figures and across runs that filtered with `--manager`, and only the
managers that ran in a scenario are drawn for it.
...
![Checks per second, discrete Contact Only scenario](docs/checks_per_second_discrete_contact_only.png)

Two things to keep in mind when reading it. **Each subplot carries its own y-axis scale** — the
`FIRST` axis here reaches 375 000 while `CLOSEST` and `ALL` stop near 150 000, so bar heights are
